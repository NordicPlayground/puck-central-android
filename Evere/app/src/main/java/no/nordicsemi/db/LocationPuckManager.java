...
public class LocationPuckManager extends EntityManager<LocationPuck> {
    ...
    public LocationPuck getPuckByUUID(String proximityUuid, int major, int minor) {
        Where constraints = new Where(
            COLUMN_PROXIMITY_UUID, Is.EQUAL, proximityUuid)
            .and(COLUMN_MAJOR, Is.EQUAL, major)
            .and(COLUMN_MINOR, Is.EQUAL, minor);
        return readFirst(select().where(constraints));
    }
}
